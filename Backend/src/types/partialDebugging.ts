...
export interface SourceGraphEdge {
  ...
  kind: 'data' | 'result' | 'assignment' | 'constraint-relation' | 'component-input' | 'component-output' | 'mock-boundary' | 'control-dependency' | 'loop-carried';
  operandIndex?: number;
  operator?: '<==' | '==>' | '<--' | '-->' | '===' | '=';
  label?: string;
...
export interface SourceLoopDto {
  id: string; header: string; iterator: string; iterationLabel: string; iterationCount?: number; parentLoopId?: string;
  bodyStatementIds: string[]; stateVariables: SourceLoopStateDto[]; sourceSpan: SourceSpan;
}
...
export interface ConstraintLoopClusterDto {
  id: string; sourceLoopId: string; label: string; constraintNodeIds: string[]; confidence: 'high' | 'medium' | 'low';
}
...
export interface ConstraintGraphDto {
  ...
  signalGroups: ConstraintSignalGroupDto[];
  loopClusters: ConstraintLoopClusterDto[];
